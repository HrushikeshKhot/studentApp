...
exports.createValidate = function () {
    return [
        ...
        body('FNAME').notEmpty().withMessage('Student  first name should not be empty'),
        body('LNAME').notEmpty().withMessage('Student  last name should not be empty'),
...
exports.get = (req, res) => {
    ...
    try {
        mm.executeDQL('select count(*) as cnt from ' + viewStudentMaster + ' where 1 ' + countCriteria, [], supportKey, (error, results1) => {
            if (error) {
                console.log(error);
                res.send({
                    "code": 400,
                ...
                mm.executeDQL('select * from ' + viewStudentMaster + ' where 1 ' + criteria, [], supportKey, (error, results) => {
                    if (error) {
                        console.log(error);
                        res.send({
                            "code": 400,
...
exports.create = (req, res) => {
    ...
    if (!errors.isEmpty()) {
        ...
        try {
             mm.executeDQL('INSERT INTO ' + studentMaster + ' SET ?', data, supportKey, (error, results) => {
                if (error) {
                console.log(error);
                res.send({
                    "code": 400,
...
exports.update = (req, res) => {
    ...
    if (!errors.isEmpty()) {
        ...
        try {
            mm.executeDQL(`UPDATE ` + studentMaster + ` SET ${setData} CREATED_MODIFIED_DATE = '${systemDate}' where ID = ${criteria.ID} `, recordData, supportKey, (error, result2) => {
                if (error) {
                    console.log(error);
                    res.send({
                        "code": 400,
